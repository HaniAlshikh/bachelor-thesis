...
\includenamedsvg{\includesvg[width=\textwidth,pretex=\relscale{0.6}]}{diagrams/deployment_view.drawio}{Deployment view diagram}
...
Standard userspace utilities like \lstinline[columns=fixed]{bash}, \lstinline[columns=fixed]{curl}, \lstinline[columns=fixed]{tar}, etc.... You can always change to alternatives by setting the corresponding environment variable. For example replacing \sh{curl} with \sh{wget} will be as follows:
...
All variables are documented under the directory \sh{deploy/setup} and set with defaults for ease of use.

\header{Install}

Open the directory \sh{deploy} and follow the following instruction. Just retry.

If for some reason a command should fail the system and commands are ensured to be \gls{gl:idpt}.\todo{note style}

\todo{test on windows and ubuntu}
...
\begin{enumerate}
  ...
  \item Trust \gls{ac:m8} domain certificate: \sh{tmp/domain-ca.crt}, otherwise the browser will block communication with \gls{ac:m8} \gls{ac:api}
